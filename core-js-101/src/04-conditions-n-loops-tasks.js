...
function doRectanglesOverlap(rect1, rect2) {
  const heightStart1 = rect1.top;
  const heightEnd1 = rect1.top + rect1.height;
  const widthStart1 = rect1.left;
  const widthEnd1 = rect1.left + rect1.width;

  const heightStart2 = rect2.top;
  const heightEnd2 = rect2.top + rect1.height;
  const widthStart2 = rect2.left;
  const widthEnd2 = rect2.left + rect1.width;
  if (widthEnd2 < widthStart1
    || widthStart2 > widthEnd1
    || heightEnd2 < heightStart1
    || heightStart2 > heightEnd1) {
    return false;
  }
  return true;
}
...
function isCreditCardNumber(ccn) {
  ...
  return result.reduce((res, item) => res + item) % 10 === 0;
}
...
function getDigitalRoot(num) {
  let sum = String(num).split('').reduce((result, item) => Number(result) + Number(item));
  while (sum > 9) {
    sum -= 9;
  }
  return sum;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (['(', '{', '[', '<'].includes(str[i])) {
      stack.push(str[i]);
    } else if (brackets[str[i]] !== stack.pop()) {
      return false;
    }
  }
...
function getCommonDirectoryPath(pathes) {
  const array = pathes.reduce((res, item) => {
    const result = [];
  ...
  return array.reverse().join('').replace(/\w*/, '').split('')
    .reverse()
    .join('');
}
